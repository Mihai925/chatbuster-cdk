...
export class ComputeConstruct extends Construct {
  ...
  constructor(scope, id, props) {
    ...
    const { vpc, dbInstance, dbSecurityGroup, secrets, certificate, deploymentBucket, shopifyApiKey } = props;
    ...
    secrets.embeddingsApiKeySecret.grantRead(role);
    ...
    secrets.stripeSecret.grantRead(role);
    secrets.resendSecret.grantRead(role);
    secrets.adminSecret.grantRead(role);
    secrets.shopifyApiSecret.grantRead(role);
    ...
    const finalScript = userDataScript
      ...
      .replace(/__EMBEDDINGS_SECRET_ARN__/g, secrets.embeddingsApiKeySecret.secretArn)
      ...
      .replace(/__STRIPE_SECRET_ARN__/g, secrets.stripeSecret.secretArn)
      .replace(/__RESEND_SECRET_ARN__/g, secrets.resendSecret.secretArn)
      .replace(/__ADMIN_SECRET_ARN__/g, secrets.adminSecret.secretArn)
      .replace(/__SHOPIFY_API_SECRET_ARN__/g, secrets.shopifyApiSecret.secretArn)
    ...
    // SECTION 2 (100k-page tier) — HELD, deploy with the RDS upgrade when the
    // first 100k-page customer onboards. SMALL -> MEDIUM (2 GiB -> 4 GiB, 2 vCPU)
    // gives the JSDOM/Readability scrape parse CPU/RAM headroom. CPU autoscaling
    // (scaleOnCpuUtilization below, max 3) is safe to keep because the Section 1
    // scrape drainer claims work atomically from the DB — multiple instances no
    // longer double-scrape or lose in-memory queue state.
    ...
    // Instance Maintenance Policy: force launch-before-terminate on ALL
    // replacements (spot interruptions, EC2 health-check failures, etc.), not
    // just CI-driven instance refreshes. With desiredCapacity=1, Min=100 alone
    // does NOT trigger launch-before-terminate — Max must exceed 100 so the
    // ASG can temporarily run 2 instances during replacement.
    this.asg.node.defaultChild.instanceMaintenancePolicy = {
